...
export default function AuthModule({ initialMode = 'login' }) {
  ...
  const [loggedInStudent, setLoggedInStudent] = React.useState(null);
  ...
  const handleSubmit = (e) => {
    ...
    if (isLogin) {
      const displayName = formData.email.split('@')[0] || 'RVU Student';
      setLoggedInStudent(displayName);
    } else {
  ...
  if (loggedInStudent) {
    return (
      <StudentPortal
        ...
        onBack={() => setLoggedInStudent(null)}
      />
    );
  }
